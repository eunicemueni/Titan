...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  return (
    <div className="p-10 lg:p-20 max-w-[1600px] mx-auto space-y-12">
      
      {/* PERSONNEL SWITCHER - Made more subtle */}
      <div className="bg-slate-900/20 border border-white/5 p-2 rounded-2xl flex items-center gap-4 overflow-x-auto no-scrollbar">
        <span className="text-[8px] font-black text-slate-600 uppercase tracking-widest ml-4 shrink-0">Persona Node:</span>
        <div className="flex gap-2">
          {profiles.map((p, idx) => (
            <button
              key={idx}
              onClick={() => onSwitchProfile(idx)}
              className={`flex items-center gap-3 px-4 py-2 rounded-xl transition-all border shrink-0 ${
                activeIndex === idx 
                ? `${p.themeColor === 'cyan' ? 'bg-cyan-600/20 border-cyan-400 text-cyan-400' : 'bg-indigo-600/20 border-indigo-400 text-indigo-400'}` 
                : 'bg-black/40 border-slate-800 text-slate-600 hover:text-slate-400'
              }`}
            >
              <div className="w-5 h-5 rounded-full overflow-hidden border border-white/10 bg-slate-900">
                <img src={`https://api.dicebear.com/7.x/bottts/svg?seed=${p.fullName}`} alt="ava" />
              </div>
              <span className="text-[9px] font-black uppercase tracking-widest">{p.fullName.split(' ')[0]}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-10">
        <div className={`xl:col-span-8 bg-slate-950 border ${profile.themeColor === 'cyan' ? 'border-cyan-500/20 shadow-sm' : 'border-indigo-500/20 shadow-sm'} rounded-[3rem] p-12 relative overflow-hidden group`}>
          <div className="flex flex-col md:flex-row items-center gap-10 relative z-10">
            <div className={`w-24 h-24 rounded-3xl p-1 ${profile.themeColor === 'cyan' ? 'bg-cyan-600/10 border border-cyan-500/30' : 'bg-indigo-600/10 border border-indigo-500/30'}`}>
              <img src={`https://api.dicebear.com/7.x/bottts/svg?seed=${profile.fullName}`} alt="Avatar" className="w-full h-full rounded-2xl" />
            </div>
            <div className="flex-1 space-y-2">
              <h1 className="text-4xl font-black text-white italic tracking-tighter uppercase leading-none">{profile.fullName}</h1>
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{profile.domain} specialist | Velocity: {targetDailyCap.toLocaleString()} dispatches/day</p>
              
              <div className="bg-black/40 border border-white/5 p-6 rounded-3xl space-y-4 mt-6">
                <div className="flex justify-between items-center">
                  <p className="text-[8px] font-black text-slate-600 uppercase tracking-widest">Adjust Target</p>
                  <span className={`text-base font-black font-mono italic ${themeText}`}>{targetDailyCap.toLocaleString()} Applications</span>
                </div>
                <input 
        ...
        <div className="xl:col-span-4 bg-slate-950 border border-white/5 rounded-[3rem] p-12 flex flex-col justify-center space-y-6">
          <div className="flex justify-between items-center">
            <h3 className="text-[9px] font-black text-slate-500 uppercase tracking-[0.3em]">System_Concurrency</h3>
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              ...
              <div key={i} className="bg-black/40 border border-white/5 p-4 rounded-xl flex items-center justify-between">
                ...
                <div className={`w-1.5 h-1.5 rounded-full ${node.active ? node.color + ' shadow-[0_0_5px_currentColor]' : 'bg-slate-900'}`}></div>
              </div>
            ))}
          </div>

          <button onClick={onToggleAutopilot} className={`w-full py-4 rounded-2xl text-[9px] font-black uppercase tracking-[0.2em] transition-all border ${isAutopilot ? 'bg-red-500/10 text-red-500 border-red-500/20' : themeBg + ' text-white ' + themeBorder}`}>{isAutopilot ? 'Stop Scrapers' : 'Engage Scrapers'}</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {[
          ...
          <button key={i} onClick={() => onNavigate(kpi.view)} className="bg-slate-950 border border-white/5 p-8 rounded-[2rem] text-left hover:border-white/20 transition-all">
            <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-2">{kpi.label}</p>
            <h3 className={`text-3xl font-black ${kpi.color} italic tracking-tighter mb-2`}>{kpi.value}</h3>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-6">
        {coreModules.map((mod) => (
          <button key={mod.id} onClick={() => onNavigate(mod.id)} className={`p-8 rounded-[2.5rem] border bg-slate-950/40 backdrop-blur-md transition-all text-left flex flex-col justify-between h-48 group shadow-sm ${mod.color} relative overflow-hidden`}>
            <div className="w-10 h-10 bg-white/5 rounded-xl flex items-center justify-center mb-6 group-hover:bg-white/10 transition-colors"><svg className="w-5 h-5 text-slate-500 group-hover:text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={mod.icon} /></svg></div>
            <div><h4 className="text-xs font-black text-white uppercase tracking-widest mb-1 leading-none">{mod.name}</h4><p className="text-[8px] text-slate-600 font-bold uppercase tracking-tight leading-tight">{mod.desc}</p></div>
          </button>
        ))}
      </div>
    </div>
