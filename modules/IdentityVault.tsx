    ...
    <div className="min-h-screen bg-[#02040a] p-4 md:p-10 lg:p-20 space-y-8 animate-in fade-in duration-700 pb-40">
      <div className="flex flex-col md:flex-row justify-between items-center gap-6 bg-slate-950/80 p-6 rounded-[2.5rem] border border-white/5 backdrop-blur-md">
        ...
        <div className="flex gap-4 w-full md:w-auto">
          ...
          {editMode ? (
            <button onClick={handleSyncToCloud} disabled={isSyncing} className="flex-1 md:flex-none px-8 py-4 bg-emerald-600 text-white border border-emerald-400 rounded-2xl font-black uppercase text-[10px] tracking-[0.3em] shadow-lg">
      ...
      {viewMode === 'DNA_STREAM' ? (
           ...
           <div className="bg-slate-950 border border-indigo-500/20 rounded-[4rem] overflow-hidden shadow-2xl">
              <div className="bg-indigo-600/10 p-10 border-b border-white/5 flex justify-between items-center">
                 <div>
                    ...
                    <p className="text-[9px] font-black text-indigo-400 uppercase tracking-widest mt-2">Active: {activeProfile.fullName}</p>
                 </div>
                 <div className="w-3 h-3 rounded-full bg-indigo-500 animate-pulse"></div>
              </div>
              <div className="p-10 md:p-20 bg-black/40 backdrop-blur-3xl min-h-[600px]">
        ...
        <div className="max-w-6xl mx-auto space-y-12 animate-in fade-in duration-500">
           <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                ...
                { label: 'Portfolio', value: activeProfile.portfolioUrl, icon: 'M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9' },
                ...
                <div key={i} className="bg-slate-950 border border-white/5 p-8 rounded-[3rem] shadow-2xl">
           ...
           <div className="bg-white rounded-[4rem] shadow-2xl overflow-hidden">
              <div className="bg-slate-950 p-12 md:p-20 text-center space-y-4">
                 <h2 className="text-[10px] font-black text-indigo-500 uppercase tracking-[0.6em]">Master Identity Dossier</h2>
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 animate-in fade-in">
           <div className="lg:col-span-4 space-y-8">
              <div className="bg-slate-950 border border-white/5 p-12 rounded-[3rem] text-center space-y-6">
              ...
              <div className="bg-slate-950 border border-white/5 p-8 rounded-[3rem] space-y-6">
                 {['email', 'portfolioUrl', 'linkedinUrl'].map((field) => (
                    <div key={field} className="space-y-2">
                       ...
                       <input type="text" className="w-full bg-black border border-slate-800 rounded-2xl px-6 py-4 text-[11px] font-mono text-white outline-none focus:border-indigo-500" value={(activeProfile as any)[field] || ''} onChange={e => handleUpdateActiveProfile({ [field]: e.target.value })} />
                    </div>
                 ))}
              </div>
           </div>
           <div className="lg:col-span-8 space-y-8">
              <div className="bg-slate-950 border border-white/5 p-12 rounded-[4rem] space-y-8">
                 <h3 className="text-xl font-black text-white uppercase italic tracking-tighter leading-none">Master DNA Core (CV)</h3>
                 <textarea className="w-full h-[600px] bg-black border border-slate-800 rounded-[2.5rem] p-10 text-[13px] font-mono leading-relaxed text-indigo-100 resize-none outline-none focus:border-indigo-500" value={activeProfile.masterCV} onChange={e => handleUpdateActiveProfile({ masterCV: e.target.value })} />
              </div>
           </div>
        </div>
      )}
