...
const GlobalSearchModal: React.FC<GlobalSearchModalProps> = ({
  ...
  useEffect(() => {
    ...
    fetch('https://titanmmmt.onrender.com/api/jobs')
      .then(res => res.json())
  ...
  return (
    <div
      ...
      onClick={e => e.target === e.currentTarget && onClose()} // only close on outside click
    >
      <div className="w-full max-w-5xl bg-[#0d1117] border border-white/10 rounded-[2.5rem] shadow-2xl flex flex-col max-h-[85vh] overflow-hidden">
        ...
        <div className="px-10 py-8 border-b border-white/5 flex justify-between items-center bg-white/2">
          <div>
            ...
            <p className="text-[8px] font-black text-indigo-500 uppercase tracking-widest mt-1">
              Pulse scan identified {results.total} matching nodes.
            </p>
          </div>
        ...
        {/* Scrollable Results */}
        <div
          className="flex-1 overflow-y-auto p-10 custom-scrollbar space-y-12 bg-black/20"
            ...
            <section className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {results.jobs.map(j => (
                  <button
                    key={j.id}
                    onClick={() => navigateTo(AppView.JOB_SCANNER)}
                    className="text-left bg-white/2 border border-white/5 p-6 rounded-3xl hover:border-indigo-500/40 transition-all flex items-center gap-4 group"
                  >
              ...
              <div className="space-y-2">
                {results.history.map(r => (
                  <button
                    ...
                    className="w-full text-left bg-white/2 border border-white/5 px-6 py-4 rounded-2xl hover:bg-white/5 transition-all flex items-center justify-between"
                  >
                    <div>
        ...
        <div className="px-10 py-6 border-t border-white/5 bg-black/40 flex justify-center shrink-0">
          <button
            onClick={onClose}
            className="px-12 py-3 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-500 transition-all shadow-xl"
          >
            Abort Pulse
          </button>
        </div>
